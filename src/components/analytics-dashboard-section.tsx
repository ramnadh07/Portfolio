...
import { Users, Target, Star, TrendingUp, Handshake, Zap, UsersRound, Activity, Loader2 } from "lucide-react";
  ...
  return (
    <AnimatedSection id="analytics" className="scroll-mt-20 md:scroll-mt-24" delay="delay-550">
      <Card className="shadow-xl border border-border/50 rounded-lg overflow-hidden bg-gradient-to-br from-card via-card/95 to-card">
        ...
        <CardContent className="p-6 md:p-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8"> {/* Adjusted grid columns to 4 */}

          {/* KPI Cards Row - Adjusted spans */}
          ...
          <AnimatedSection animationClass="animate-fade-in-up" delay={cardAnimationDelay(1)} className="col-span-1">
           ...
           {/* Viewers Over Time */}
           <AnimatedSection animationClass="animate-fade-in-up" delay={cardAnimationDelay(3)} className="col-span-1 sm:col-span-2 lg:col-span-1"> {/* Span 1 column on all sizes */}
              <Card className="h-full shadow-md hover:shadow-lg transition-shadow duration-300 border border-border/40 bg-background/70 flex flex-col">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          {/* Skills Interest */}
          <AnimatedSection animationClass="animate-fade-in-up" delay={cardAnimationDelay(4)} className="col-span-1 sm:col-span-2 lg:col-span-2"> {/* Adjusted span */}
               <Card className="h-full shadow-md hover:shadow-lg transition-shadow duration-300 border border-border/40 bg-background/70 flex flex-col">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <CardContent className="flex-grow pt-2"> {/* Use flex-grow */}
                  ...
                  <p className="text-xs text-muted-foreground mb-2">Most frequently mentioned</p>
                   <div className="flex-grow h-[120px]"> {/* Set a fixed height */}
         ...
         <AnimatedSection animationClass="animate-fade-in-up" delay={cardAnimationDelay(5)} className="col-span-1">
              <Card className="h-full shadow-md hover:shadow-lg transition-shadow duration-300 border border-border/40 bg-background/70 flex flex-col">
                ...
                <CardContent className="flex-grow flex flex-col items-center justify-center pt-2"> {/* Reduced top padding */}
                    ...
                    <div className="h-[80px] w-[80px]"> {/* Adjusted size */}
                        <ChartContainer config={domainInterestData.reduce((acc, cur) => ({ ...acc, [cur.name]: { label: cur.name, color: cur.fill } }), {})}>
                            <PieChart margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                            <ChartTooltip content={<ChartTooltipContent nameKey="name" hideIndicator />} />
                            <Pie data={domainInterestData} dataKey="value" nameKey="name" innerRadius={20} outerRadius={35} strokeWidth={1} >
           ...
           {/* Needs Summary */}
          <AnimatedSection animationClass="animate-fade-in-up" delay={cardAnimationDelay(6)} className="col-span-1 sm:col-span-2 lg:col-span-4"> {/* Span full width */}
              <Card className="shadow-md hover:shadow-lg transition-shadow duration-300 border border-border/40 bg-background/70">
                ...
                <CardContent className="h-[150px] pt-4">
                    <ChartContainer config={needsData.reduce((acc, cur) => ({ ...acc, [cur.name]: { label: cur.name, color: cur.fill } }), {})}>
                        <BarChart data={needsData} layout="horizontal" margin={{ top: 5, right: 10, left: 10, bottom: 0 }}>
